...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetchWithRetry(`${SUPABASE_URL}/functions/v1/sync-county-harris`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      results.sync = { error: `Harris County sync failed (${res.status}): ${errText}` }
    } else {
  ...
  // Phase 2: Classify new entities server-side (Next.js has no 60s limit like edge functions)
