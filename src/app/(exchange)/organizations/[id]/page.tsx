...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  var { data } = await supabase.from('organizations').select('org_name, description_5th_grade').eq('org_id', id).single()
...
export default async function OrganizationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  var fullAddress = [org.address, org.city, org.state, org.zip_code].filter(Boolean).join(', ')

  // Parse social media if present
  var socialLinks: Array<{ platform: string; url: string }> = []
  if (org.social_media) {
    try {
      var sm = typeof org.social_media === 'string' ? JSON.parse(org.social_media) : org.social_media
  ...
  var hoursDisplay: string | null = null
  if (org.hours_of_operation) {
    try {
      var hours = typeof org.hours_of_operation === 'string' ? JSON.parse(org.hours_of_operation) : org.hours_of_operation
