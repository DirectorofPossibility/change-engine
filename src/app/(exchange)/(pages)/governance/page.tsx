      ...
      <div className="bg-paper relative overflow-hidden border-b">
        ...
        <div className="max-w-[900px] mx-auto px-6 py-12 relative">
          ...
          <div className="flex gap-6" style={{ fontSize: 12, color: "#5c6474" }}>
            <span><strong style={{  }}>{allOfficials.length}</strong> Officials</span>
            <span><strong style={{  }}>{allPolicies.length}</strong> Policies Tracked</span>
            <span><strong style={{  }}>{LEVELS.length}</strong> Levels</span>
          </div>
        </div>
      </div>
          ...
          return (
            <section key={level.key}>
              ...
              <div className="flex items-baseline justify-between mb-1">
                <h2 style={{ fontSize: 24 }}>{level.label}</h2>
              </div>
              <div style={{ height: 1, background: '#dde1e8' }} className="mb-1" />
              <p style={{ color: "#5c6474", fontSize: 11 }} className="mb-5">
              ...
              {lvlOfficials.length > 0 && (
                <div className="mb-6">
                  <p style={{ fontSize: 10, color: "#5c6474", letterSpacing: '0.1em' }} className="uppercase mb-3">
                    Who represents you
                  </p>
                      ...
                      return (
                        ...
                        >
                          {o.photo_url ? (
                            ...
                            <div className="h-[50px] flex items-center justify-center bg-paper">
                              <span style={{ color: "#5c6474", fontSize: 20 }}>{o.official_name.charAt(0)}</span>
                            </div>
                          )}
                          <div className="p-3">
                            <p style={{ fontSize: 13 }} className="leading-tight line-clamp-1">{o.official_name}</p>
                            <p style={{ color: "#5c6474", fontSize: 10 }} className="mt-0.5 leading-snug line-clamp-2">{o.title}</p>
                            {o.party && (
                              <p style={{ color: "#1b5e8a", fontSize: 10 }} className="mt-1">{o.party}</p>
                            )}
                          </div>
                        </Link>
                      )
                  ...
                  {lvlOfficials.length > 8 && (
                    <details className="mt-2">
                      <summary style={{ fontStyle: 'italic', color: "#1b5e8a", cursor: 'pointer', fontSize: 14 }}>
                        ...
                        {lvlOfficials.slice(8).map(function (o) {
                          return (
                            ...
                            >
                              ...
                              <div className="p-3">
                                <p style={{ fontSize: 13 }} className="leading-tight line-clamp-1">{o.official_name}</p>
                                <p style={{ color: "#5c6474", fontSize: 10 }} className="mt-0.5 line-clamp-2">{o.title}</p>
                                {o.party && (
                                  <p style={{ color: "#1b5e8a", fontSize: 10 }} className="mt-1">{o.party}</p>
                                )}
                              </div>
              ...
              {lvlPolicies.length > 0 && (
                <div className="mb-4">
                  ...
                  <div className="border overflow-hidden" style={{ borderColor: '#dde1e8' }}>
                    {lvlPolicies.map(function (p, pIdx) {
                      ...
                      return (
                        <Link
                          ...
                          style={{ borderBottom: pIdx < lvlPolicies.length - 1 ? `1px solid ${'#dde1e8'}` : 'none', background: "#f4f5f7" }}
                        >
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              {p.bill_number && (
                                <span style={{ fontSize: 11, color: "#5c6474" }}>{p.bill_number}</span>
                              )}
                              {p.status && (
                            ...
                            <p style={{ fontSize: 14 }} className="leading-snug line-clamp-2">
                              {displayTitle}
                            </p>
                            {(p.summary_6th_grade || p.summary_5th_grade) && (
                              <p style={{ color: "#5c6474", fontSize: 12 }} className="mt-1 line-clamp-1">
                                {p.summary_6th_grade || p.summary_5th_grade}
                              </p>
                            )}
                          </div>
                          {p.last_action_date && (
                            <span style={{ fontSize: 10, color: "#5c6474" }} className="flex-shrink-0 mt-1 tabular-nums">
                  ...
                  {(policiesByLevel[level.key] || []).length > 12 && (
                    <div className="mt-2 text-right">
                      <Link href={'/policies?level=' + level.key} style={{ color: "#1b5e8a", fontSize: 12 }} className="hover:underline">
