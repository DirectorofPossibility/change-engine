...
function BookCard({ book, featured }: { book: BookshelfItem; featured?: boolean }) {
  ...
  return (
    ...
    >
      {/* Book spine */}
      <div className="w-2 flex-shrink-0" style={{ background: theme?.color || '#1b5e8a' }} />
      ...
      {book.cover_image_url ? (
        ...
        <div className="w-16 flex-shrink-0 flex flex-col items-center justify-center py-4 bg-brand-bg border-r border-brand-border">
          <BookOpen size={20} style={{ color: theme?.color || '#1b5e8a' }} />
          {book.year_published && (
            <span className="font-mono text-[8px] font-bold mt-1" style={{ color: theme?.color || '#1b5e8a' }}>{book.year_published}</span>
          )}
        </div>
      )}
