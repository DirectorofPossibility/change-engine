...
export function PollingPlaceClient({ activeElection }: PollingPlaceClientProps) {
  ...
  var [results, setResults] = useState<Results | null>(null)
  var [locationTypeFilter, setLocationTypeFilter] = useState<'all' | 'Early Voting' | 'Election Day'>('all')
  var [accessibilityFilter, setAccessibilityFilter] = useState(false)
  ...
  async function handleLookup(e: React.FormEvent) {
    ...
    try {
      ...
      var { data: locations } = await query
  ...
  var filtered = results ? results.votingLocations.filter(function (loc) {
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {(['all', 'Early Voting', 'Election Day'] as const).map(function (filter) {
              var isActive = locationTypeFilter === filter
              return (
                <button
                  key={filter}
