    ...
    <div>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {sdgs.map(function (s) {
            var isActive = activeSDG === s.sdg_id
            return (
              <button
                key={s.sdg_id}
      ...
      <div className="mb-8">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {sdohDomains.map(function (d) {
            var isActive = activeSDOH === d.sdoh_code
            return (
              <button
                key={d.sdoh_code}
      ...
      <div className="space-y-8">
        {themes.map(function (theme) {
          var filtered = theme.focusAreas.filter(matchesFilters)
            ...
            <div key={theme.id}>
              ...
              <div className="flex flex-wrap gap-2">
                {filtered.map(function (fa) {
                  var baseClass = 'text-xs px-3 py-1.5 rounded-full bg-white text-brand-text hover:shadow-md transition-shadow'
                  if (fa.is_bridging) {
        ...
        {(function () {
          var filtered = unthemedAreas.filter(matchesFilters)
          if (filtered.length === 0) return null
          totalVisible += filtered.length
          return (
            <div>
