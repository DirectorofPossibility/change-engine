...
export function ThemePill({ themeId, size = 'sm' }: ThemePillProps) {
  ...
  if (!theme) return <span className="text-xs bg-gray-200 px-2 py-0.5 rounded-full">{themeId}</span>

  const sizeClasses = size === 'sm' ? 'text-xs px-2 py-0.5' : 'text-sm px-3 py-1'

  return (
    <span
      className={`inline-block rounded-full text-white font-medium ${sizeClasses}`}
