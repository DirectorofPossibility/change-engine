      ...
      <form onSubmit={handleLookup} className="flex gap-3 mb-8">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-3 border border-brand-border text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-accent/20 focus:border-brand-accent"
          />
        </div>
        <button
          type="submit"
      ...
      {results && (
        <div className="space-y-10">
          ...
          {results.neighborhood && (
            <div className="bg-brand-accent/5 p-4 border border-brand-border">
          ...
          {hasOfficials && (
            <div>
              <h2 className="text-xl font-display font-bold text-brand-text mb-4 flex items-center gap-2 border-b border-brand-border pb-2">
                    ...
                    <div key={section.key}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {items.map(function (svc) {
                          return (
                            <div key={svc.id} className={'bg-white border p-4 ' + (svc.is_emergency ? 'border-red-300 bg-red-50/30' : 'border-brand-border')}>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {results.votingLocations.map(function (loc) {
                  return (
                    <div key={loc.location_id} className="bg-white border border-brand-border p-4">
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {officials.map(function (o) {
          return (
            <div key={o.official_id} className="bg-white border border-brand-border p-4">
              <div className="mb-2">
              ...
              <div className="flex items-center gap-2 mb-2">
                {o.party && <span className="text-xs bg-brand-bg px-2 py-0.5 text-brand-muted">{o.party}</span>}
                {o.level && <span className={'text-xs px-2 py-0.5 ' + levelColor(o.level)}>{o.level}</span>}
              </div>
