...
export function LifeSituationCard({ name, slug, description, urgency, iconName, translatedName, translatedDescription, onSelect }: LifeSituationCardProps) {
  ...
  const wrapperProps = onSelect
    ? { role: 'button' as const, tabIndex: 0, onClick: onSelect, onKeyDown: function (e: React.KeyboardEvent) { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSelect() } }, className: `block border overflow-hidden hover:border-ink transition-shadow cursor-pointer ${colors.bg} ${colors.border}` }
    : { href: '/help/' + (slug || ''), className: `block border overflow-hidden hover:border-ink transition-shadow ${colors.bg} ${colors.border}` }
